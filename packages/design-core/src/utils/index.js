...
export function serializeFrontmatter(obj, indent = 0) {
    const lines = [];
    ...
    for (const [key, val] of Object.entries(obj)) {
        ...
        if (typeof val === 'object' && !Array.isArray(val)) {
            ...
            lines.push(serializeFrontmatter(val, indent + 1));
        }
...
export function createDesignMarkdown(designSystem, frontmatter) {
...
export function extractTokenByPath(designSystem, path) {
...
export function computeTokenDiff(before, after) {
...
function luminanceInternal(hex) {
...
export function isWcagCompliant(color1, color2, level = 'AA') {
