...
describe('ModelHandler', () => {
  ...
  describe('symbol table from colors', () => {
    ...
    it('successfully parses 4-level nested color declarations', () => {
      ...
      expect(result.designSystem.symbolTable.has('colors.theme.surface.background.base')).toBe(
        true
      );
    });
    ...
    it('resolves functional rgb/rgba colors', () => {
      // comma separated
      const resComma = handler.execute(
        makeParsed({
      ...
      const resSpace = handler.execute(
        makeParsed({
    ...
    it('resolves color-mix colors', () => {
      const result = handler.execute(
        makeParsed({
          colors: {
            mix1: 'color-mix(in srgb, red 20%, blue 80%)',
            mix2: 'color-mix(in srgb, red, white 50%)',
          },
        })
      );
      expect(result.findings.length).toBe(0);
  ...
  describe('single-level token reference resolution', () => {
    it('resolves a direct {section.token} reference in components', () => {
      ...
      expect(typeof bg === 'object' && bg !== null && 'type' in bg && bg.type === 'color').toBe(
        true
      );
    });
  });
  ...
  describe('WCAG contrast ratio', () => {
    ...
    it('computes correct contrast for identical colors (1:1)', () => {
      ...
      const ratio = contrastRatio(
        result.designSystem.colors.get('red1')!,
        result.designSystem.colors.get('red2')!
      );
  ...
  describe('rounded token reference resolution', () => {
    ...
    it('resolved rounded reference appears in symbol table', () => {
      ...
      expect(
        typeof sym === 'object' && sym !== null && 'type' in sym && sym.type === 'dimension'
      ).toBe(true);
    });
  });
