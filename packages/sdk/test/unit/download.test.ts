...
describe('DownloadAssetsHandler', () => {
    ...
    const mockFetch = vi.fn().mockImplementation((url) => {
      if (url === 'http://fake/s1.html') {
        return Promise.resolve({
          ok: true,
          text: () => Promise.resolve('<html><img src="http://example.com/bad name.png"></html>'),
        });
      }
      return Promise.resolve({
        ok: true,
    ...
    const mockFetch = vi.fn().mockImplementation((url) => {
      if (url === 'http://fake/s1.html') {
        return Promise.resolve({
          ok: true,
          text: () =>
            Promise.resolve('<html><img src="http://example.com/%2e%2e/etc/passwd"></html>'),
        });
      }
      return Promise.resolve({
        ok: true,
  ...
  it('uses custom assetsSubdir option', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockImplementation((url) => {
        if (url === 'http://fake/s1.html') {
          return Promise.resolve({
            ok: true,
            text: () => Promise.resolve('<html><img src="http://example.com/img.png"></html>'),
          });
        }
        return Promise.resolve({
          ok: true,
    ...
    expect(fs.mkdir).toHaveBeenCalledWith(expect.stringContaining('static'), expect.anything());
  });
