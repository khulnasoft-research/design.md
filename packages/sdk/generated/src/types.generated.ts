...
/** The theme of the design. Next ID: 27 LINT.IfChange */
...
/** Metadata relating to the project. */
export interface ProjectMetadata {
  isRemixed?: boolean;
  userRole?: 'ROLE_UNSPECIFIED' | 'OWNER' | 'READER';
}

/** An instance of a screen on the project. */
...
export interface ScreenInput {
  ...
  screenType?:
    'SCREEN_TYPE_UNSPECIFIED' | 'DESIGN' | 'IMAGE' | 'PROTOTYPE' | 'DOCUMENT' | 'PROTOTYPE_V2';
  screenshot?: File;
  theme?: DesignTheme;
  title?: string;
  width?: string;
...
/** The bounding box of a component. */
...
/** The region of a component region. */
...
/** A suggested next action for a screen. */
...
export interface ProgressUpdate {
  displayName?: string;
  status?: 'STATUS_UNSPECIFIED' | 'STARTED' | 'COMPLETED' | 'FAILED';
  toolCallId?: string;
  toolName?: string;
}
...
/** A single interactive link within a prototype screen. */
...
/** A collection of prototype links for a single screen. */
export interface PrototypeLinks {
  links?: PrototypeLink[];
}

/** A named state that controls link behavior in a prototype. */
export interface PrototypeState {
  name?: string;
}

/** The full specification for a V2 interactive prototype. */
export interface PrototypeV2Spec {
  initScreenId?: string;
  links?: Record<string, PrototypeLinks>;
  screenIds?: string[];
  states?: PrototypeState[];
}

/** A single structured question the agent asks the user. */
...
/** A set of questions sent from the agent to the user. */
...
/** Flexible session events carrying JSON payload. */
...
/** A partial output of the session. */
export interface SessionOutputComponent {
  design?: Design;
  designSystem?: Asset;
